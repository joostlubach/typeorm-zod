...
export function string<T extends z.ZodString>(base: T, type?: 'varchar' | 'text', options?: ColumnOptions): StringColumn<T>
export function string<T extends z.ZodType<string>>(base: T, type?: 'varchar' | 'text', options?: ColumnOptions): Column<T>
export function string(type?: 'varchar' | 'text', options?: ColumnOptions): StringColumn<z.ZodString>
...
export class StringColumn<T extends z.ZodString = z.ZodString> extends Column<T> {
  ...
  public min(minLength: number) {
    this.zod.min(minLength)
    return this
  }

  public max(maxLength: number) {
    this.zod.max(maxLength)
